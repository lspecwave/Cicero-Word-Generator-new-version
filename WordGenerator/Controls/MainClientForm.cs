    ...
    public partial class MainClientForm : Form
    {
        ...
        public void RefreshSequenceDataToUI()
        {
            ...
            try
            {
                ...
                ApplySequenceReadOnlyModeTo(mainTab);
            }
            finally
            {
        ...
        private int sequenceReadOnlyLockCount = 0;
        private Dictionary<Control, bool> sequenceReadOnlyEnabledStates = new Dictionary<Control, bool>();
        private Dictionary<TextBoxBase, bool> sequenceReadOnlyTextBoxStates = new Dictionary<TextBoxBase, bool>();
        private Dictionary<ToolStripItem, bool> sequenceReadOnlyToolStripItemStates = new Dictionary<ToolStripItem, bool>();

        public bool SequenceEditingLocked
        {
            get { return sequenceReadOnlyLockCount > 0; }
        }

        public void ShowRunForm(SequenceData sequenceToRun, RunForm.RunType runType, bool runRepeat, bool isCameraSaving)
        {
            RunForm runForm = new RunForm(sequenceToRun, runType, runRepeat, isCameraSaving);
            runForm.SequenceRunStarted += new EventHandler(runForm_SequenceRunStarted);
            runForm.SequenceRunFinished += new EventHandler(runForm_SequenceRunFinished);
            runForm.FormClosed += new FormClosedEventHandler(runForm_FormClosed);
            runForm.Show(this);
        }

        private void runForm_SequenceRunStarted(object sender, EventArgs e)
        {
            BeginSequenceReadOnlyMode();
        }

        private void runForm_SequenceRunFinished(object sender, EventArgs e)
        {
            EndSequenceReadOnlyMode();
        }
        ...
        private void runForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            RunForm runForm = sender as RunForm;
            if (runForm != null)
            {
                runForm.SequenceRunStarted -= new EventHandler(runForm_SequenceRunStarted);
                runForm.SequenceRunFinished -= new EventHandler(runForm_SequenceRunFinished);
        ...
        private void BeginSequenceReadOnlyMode()
        {
            if (this.InvokeRequired)
            {
                this.BeginInvoke(new MethodInvoker(BeginSequenceReadOnlyMode));
                return;
            }

            sequenceReadOnlyLockCount++;
            if (sequenceReadOnlyLockCount > 1)
                return;
            ...
            ApplySequenceReadOnlyToControl(mainTab);
            ApplySequenceReadOnlyToToolStrip(menuStrip);
            toolStripStatusLabel.Text = "Sequence running: main window is view-only.";
        }

        private void EndSequenceReadOnlyMode()
        {
            if (this.InvokeRequired)
            {
                this.BeginInvoke(new MethodInvoker(EndSequenceReadOnlyMode));
                return;
            }

            if (sequenceReadOnlyLockCount == 0)
                return;
            ...
            RestoreSequenceReadOnlyMode();
            toolStripStatusLabel.Text = "Sequence editing enabled.";
        }

        public void ApplySequenceReadOnlyModeTo(Control root)
        {
            if (!SequenceEditingLocked || root == null)
                return;

            if (this.InvokeRequired)
            {
                this.BeginInvoke(new Action<Control>(ApplySequenceReadOnlyModeTo), new object[] { root });
                return;
            }

            ApplySequenceReadOnlyToControl(root);
        }

        private void ApplySequenceReadOnlyToControl(Control control)
        {
            ...
            if (textBox != null)
            {
                if (!sequenceReadOnlyTextBoxStates.ContainsKey(textBox))
                    sequenceReadOnlyTextBoxStates.Add(textBox, textBox.ReadOnly);
                textBox.ReadOnly = true;
            }

            if (ShouldDisableControlForSequenceReadOnly(control))
            {
                if (!sequenceReadOnlyEnabledStates.ContainsKey(control))
                    sequenceReadOnlyEnabledStates.Add(control, control.Enabled);
                control.Enabled = false;
            }

            if (control.ContextMenuStrip != null)
                ApplySequenceReadOnlyToToolStrip(control.ContextMenuStrip);

            foreach (Control child in control.Controls)
            {
                ApplySequenceReadOnlyToControl(child);
            }
        }

        private bool ShouldDisableControlForSequenceReadOnly(Control control)
        {
            if (control is TextBoxBase)
                return false;

            if (control is TabControl || control is TabPage || control is Panel ||
                control is SplitContainer || control is TableLayoutPanel ||
                control is FlowLayoutPanel || control is GroupBox ||
                control is Label || control is PictureBox || control is ProgressBar)
                return false;

            if (IsReadOnlyNavigationControl(control))
                return false;

            if (control is Button || control is CheckBox || control is RadioButton ||
                control is ComboBox || control is NumericUpDown || control is DomainUpDown ||
                control is DateTimePicker || control is TrackBar || control is ListBox ||
                control is CheckedListBox || control is ListView || control is TreeView ||
        ...
        private bool IsReadOnlyNavigationControl(Control control)
        {
            if (control is ComboBox)
            {
                if (control.Name == "analogGroupSelector" && IsInsideControlType(control, "AnalogGroupEditor"))
                    return true;
                if (control.Name == "gpibGroupSelector" && IsInsideControlType(control, "GpibGroupEditor"))
                    return true;
                if (control.Name == "rs232GroupSelector" && IsInsideControlType(control, "RS232GroupEditor"))
                    return true;
            }

            if (control is Button)
            {
                if ((control.Name == "plus" || control.Name == "minus") &&
                    (IsInsideControlType(control, "AnalogGroupEditor") ||
                     IsInsideControlType(control, "GpibGroupEditor") ||
                     IsInsideControlType(control, "RS232GroupEditor")))
                    return true;

                if (control.Name == "analogPreviewUpdate")
                    return true;
            }
        ...
        private bool IsInsideControlType(Control control, string typeName)
        {
        ...
        private void ApplySequenceReadOnlyToToolStrip(ToolStrip toolStrip)
        {
            if (toolStrip == null)
                return;

            foreach (ToolStripItem item in toolStrip.Items)
                ApplySequenceReadOnlyToToolStripItem(item);
        }

        private void ApplySequenceReadOnlyToToolStripItem(ToolStripItem item)
        {
            if (item == null)
                return;

            if (ShouldDisableToolStripItemForSequenceReadOnly(item))
            {
                if (!sequenceReadOnlyToolStripItemStates.ContainsKey(item))
                    sequenceReadOnlyToolStripItemStates.Add(item, item.Enabled);
                item.Enabled = false;
            }
            ...
            if (dropDownItem != null)
            {
                foreach (ToolStripItem child in dropDownItem.DropDownItems)
                    ApplySequenceReadOnlyToToolStripItem(child);
            }
        }

        private bool ShouldDisableToolStripItemForSequenceReadOnly(ToolStripItem item)
        {
            if (item == aboutToolStripMenuItem || IsUnderToolStripItem(item, aboutToolStripMenuItem))
                return false;

            return true;
        }

        private bool IsUnderToolStripItem(ToolStripItem item, ToolStripItem possibleParent)
        {
            ...
            while (current != null)
            {
                ...
                ToolStripDropDown ownerDropDown = current.Owner as ToolStripDropDown;
                if (ownerDropDown == null)
                    return false;

                current = ownerDropDown.OwnerItem;
            }

            return false;
        }

        private void RestoreSequenceReadOnlyMode()
        {
            foreach (KeyValuePair<TextBoxBase, bool> pair in sequenceReadOnlyTextBoxStates)
            {
                try
                {
                    if (!pair.Key.IsDisposed)
                        pair.Key.ReadOnly = pair.Value;
                }
                catch
                {
                }
            ...
            foreach (KeyValuePair<Control, bool> pair in sequenceReadOnlyEnabledStates)
            {
                try
                {
                    if (!pair.Key.IsDisposed)
            ...
            foreach (KeyValuePair<ToolStripItem, bool> pair in sequenceReadOnlyToolStripItemStates)
            {
                try
                {
                    pair.Key.Enabled = pair.Value;
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            switch (m.Msg)
            {
                ...
                case WM_HOTKEY:
                    {
                        int id = (int)m.WParam;
                        if (id < 0 || id >= hotKeyBindings.Count)
                            break;
        ...
        private void runIteration0ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!suppressHotkeys)
            {
                ShowRunForm(Storage.sequenceData, RunForm.RunType.Run_Iteration_Zero, sequencePage.runControl1.repeatCheckBox.Checked, true);
            }
        ...
        private void runCurrentIterationToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!suppressHotkeys)
            {
                ShowRunForm(Storage.sequenceData, RunForm.RunType.Run_Current_Iteration, sequencePage.runControl1.repeatCheckBox.Checked, true);
            }
        }
        ...
        private void runListToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!suppressHotkeys)
            {
                ShowRunForm(Storage.sequenceData, RunForm.RunType.Run_Full_List, sequencePage.runControl1.repeatCheckBox.Checked, true);
            }
        }
        ...
        private void continueListToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!suppressHotkeys)
            {
                ShowRunForm(Storage.sequenceData, RunForm.RunType.Run_Continue_List, sequencePage.runControl1.repeatCheckBox.Checked, true);
            }
        }
        ...
        private void runListInRandomOrderToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!suppressHotkeys)
            {
                ShowRunForm(Storage.sequenceData, RunForm.RunType.Run_Random_Order_List, sequencePage.runControl1.repeatCheckBox.Checked, true);
            }
        }
        ...
        private void runWithoutSavingToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!suppressHotkeys)
            {
                ShowRunForm(Storage.sequenceData, RunForm.RunType.Run_Iteration_Zero, sequencePage.runControl1.repeatCheckBox.Checked, false);
            }
        }
	 	...
	 	private void compareSequenceMenuItem_Click(object sender, EventArgs e)
	        {
	            try
	            {
                    string basePath;
                    string comparedPath;
	                SequenceData baseSequence = LoadSequenceForComparison("Open Base Sequence", out basePath);
                    if (baseSequence == null)
                        return;

                    SequenceData comparedSequence = LoadSequenceForComparison("Open Sequence to Compare", out comparedPath);
                    if (comparedSequence == null)
                        return;

	                List<SequenceComparer.SequenceDifference> differences = SequenceComparer.CompareSequences(
	                    baseSequence, comparedSequence);
	                SequenceDifferencesForm frm = new SequenceDifferencesForm(differences, basePath, comparedPath);
	                frm.ShowDialog();
	            }
	            catch (Exception ex)
	            {
	                MessageBox.Show("Caught an exception when attempting to compare the sequences. Please copy the following exception information and send it to the author. Cicero is not about to crash.");
        ...
        private SequenceData LoadSequenceForComparison(string title, out string path)
        {
            OpenFileDialog openFileDialog = new OpenFileDialog();
            openFileDialog.Title = title;
            openFileDialog.Filter = FileNameStrings.fileDialogFilterString(
                FileNameStrings.FriendlyNames.SequenceData,
                FileNameStrings.Extensions.SequenceData);
            ...
            SequenceData sequence = Common.loadBinaryObjectFromFile(path) as SequenceData;
            if (sequence == null)
            {
                MessageBox.Show("The selected file is not a Cicero sequence file.");
                return null;
            }
