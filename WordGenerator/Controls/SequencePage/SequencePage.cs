...
namespace WordGenerator.Controls
{
    public partial class SequencePage : UserControl
    {
        ...
        private void layoutTimestepEditorPositions()
        {
            int x = 0;
            int maxHeight = TimestepEditor.TimestepEditorHeight;

            foreach (TimeStep step in Storage.sequenceData.TimeSteps)
            {
                TimestepEditor editor = getTimestepEditor(step);
                if (editor == null)
                    continue;

                bool visible = !hideHiddenTimesteps || !step.StepHidden;
                editor.Visible = visible;
                editor.TabIndex = Storage.sequenceData.TimeSteps.IndexOf(step);

                if (visible)
                {
                    editor.Location = new Point(x, 0);
                    x += editor.Width;
                    maxHeight = Math.Max(maxHeight, editor.Height);
                }
            }
            ...
            timeStepsFlowPanel.Size = new Size(x, maxHeight);
        }
        ...
        public void layoutAll()
        {
            ...
            syncHideHiddenTimestepsSetting();
            layoutTimestepEditors();
            layoutSettingsData();
            layoutTheRest();
        ...
        private void syncHideHiddenTimestepsSetting()
        {
            hideHiddenTimesteps = Storage.sequenceData.stepHidingEnabled;

            if (hideHiddenTimestepsCheckbox.Checked != hideHiddenTimesteps)
            {
                hideHiddenTimestepsCheckbox.CheckedChanged -= checkBox1_CheckedChanged;
                hideHiddenTimestepsCheckbox.Checked = hideHiddenTimesteps;
                hideHiddenTimestepsCheckbox.CheckedChanged += checkBox1_CheckedChanged;
            }
        }
        ...
        public void updateTimestepEditorsAfterSequenceModeOrTimestepGroupChange()
        {
            ...
            foreach (Control con in timeStepsFlowPanel.Controls)
            {
                ...
                if (te != null)
                {
                    ...
                    bool shouldBeVisible = !hideHiddenTimesteps || !te.StepData.StepHidden;
                    if (te.Visible != shouldBeVisible)
                    {
                        te.Visible = shouldBeVisible;
   ...
   //             throw new Exception("Microsoft sucks.");
        ...
        {
            if (WordGenerator.MainClientForm.instance != null && WordGenerator.MainClientForm.instance.SequenceEditingLocked)
                return;
