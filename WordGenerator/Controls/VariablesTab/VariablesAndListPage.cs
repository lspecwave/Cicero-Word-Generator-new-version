    ...
    public partial class VariablesAndListPage : UserControl
    {
        ...
        public void layout()
        {
            ...
            try
            {
                ...
                if (MainClientForm.instance != null)
                    MainClientForm.instance.ApplySequenceReadOnlyModeTo(this);
        ...
        private void sortVariablesButton_Click(object sender, EventArgs e)
        {
            if (Storage.sequenceData == null || Storage.sequenceData.Variables == null)
                return;

            List<Variable> variables = Storage.sequenceData.Variables;
            List<Variable> normalVariables = new List<Variable>();
            Dictionary<Variable, int> originalVisibleIndex = new Dictionary<Variable, int>();

            foreach (Variable var in variables)
            {
                if (var != null && !var.IsSpecialVariable)
                {
                    originalVisibleIndex[var] = normalVariables.Count;
                    normalVariables.Add(var);
                }
            }

            normalVariables.Sort(delegate(Variable x, Variable y)
            {
                string xName = x == null ? "" : x.VariableName;
                string yName = y == null ? "" : y.VariableName;

                int comparison = StringComparer.CurrentCultureIgnoreCase.Compare(xName, yName);
                if (comparison != 0)
                    return comparison;

                comparison = StringComparer.CurrentCulture.Compare(xName, yName);
                if (comparison != 0)
                    return comparison;

                return originalVisibleIndex[x].CompareTo(originalVisibleIndex[y]);
            });

            int sortedIndex = 0;
            for (int i = 0; i < variables.Count; i++)
            {
                // Special variables have fixed internal meaning; preserve their slots exactly.
                if (variables[i] != null && !variables[i].IsSpecialVariable)
                {
                    variables[i] = normalVariables[sortedIndex];
                    sortedIndex++;
                }
            }

            this.layoutVariables();
            this.ved_valueChanged(this, null);
            if (MainClientForm.instance != null)
                MainClientForm.instance.ApplySequenceReadOnlyModeTo(this);
        }
